...
However the two executables were compiled with the same stack frames size, $n=2^-$.
...
For our different experimentations we had \textcolor{green}{number} different C programs which were present within the CompCert project for testing purposes.
The performances of our approach are compared with the original CompCert project whose performances are between \texttt{gcc -O0} and \texttt{gcc -O1}.
...
The results are presented in the following graph Figure~\ref{efficiency_graph}.
For better comprehension among all the programs we tested we removed a part of them which execution time were too short to have a relevant comparison.

\textcolor{green}{Analysis of the graph}

\paragraph{Number of runtime checks added}

\paragraph{Stack frames size}
\paragraph{Relation betw}
