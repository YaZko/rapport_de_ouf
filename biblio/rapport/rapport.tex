...
%
%Des optimisations sont également proposées pour réduire les baisses de performance engendrées par le \textit{sandboxing}. Ces trois optimisations sont reprises dans les travaux ultérieurs sur SFI et sont donc considéré comme faisant partie de l'approche traditionnelle.
%
%\textbf{Registre spécial} Le registre \texttt{\%ebp} ou \textit{frame pointer} est utilisé très régulièrement utilisé pour accéder à la pile. De ce fait au lieu d'introduire les opérations de \textit{sandboxing} à chaque fois il est préférable de protéger le registre uniquement lorsqu'il est modifié. Ainsi le registre \texttt{\%ebp} est alors traité comme un registre dédié même si il est encore disponible pour l'exécution du code.
%
%\textbf{Gardes de régions} L'optimisation précédente serait optimale si \texttt{\%ebp} était utilisé en tant que tel cependant il est surtout utilisé avec un biais pour pouvoir accéder aux variables locales de la pile. à la place de \textit{sandboxé} ces valeurs, SFI propose de mettre des zones de gardes aux extrémités des segments de mémoire. Ces gardes lorsqu'elle seraient accédées provoqueraient une faute d'accès mémoire et empêcheraient d'accéder à des adresses illégales dans le cas d'une valeur de registre avec biais.
%
%\textbf{\textit{Sandbox} plutôt que \textit{segment matching}}. Cette optimisation a déjà été évoquée auparavant et prône le fait que si on vérifiait la valeur des tags au lieu de la remplacer systématiquement l'opération apporterait de la complexité au programme. En effet il faudrait faire un branchage dans le cas où le tag est invalide et dans l'hypothèse où ces fautes n'arrivent que rarement on privilégie le \textit{sandboxing}. Cependant dans certaines utilisations comme le débuguage il serait intéressant d'utiliser le \textit{segment matching} qui a l'avantage de pouvoir indiquer la location des bugs.
